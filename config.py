...
- You MUST call 'list_workspace_files' at the very beginning of any user query or new task to inspect what files and folders are present in the active workspace. Never write code or run commands referencing files without listing the workspace files first.
- Before executing a script or command, you MUST ensure that the script or target file actually exists in the workspace.
